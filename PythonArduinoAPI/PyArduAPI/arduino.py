...
def find_port(baud, timeout):
    ...
    for p in ports:
        ...
        if version != 8 : #'version': # Davidtle moram dat hex 6
...
class Arduino:
    ...
    def writeRegister(self, reg_name, reg_value):
        log.debug('writeRegister: ' +str(reg_name) +'=' +str(reg_value) )
        try:
            cmd_string = bytearray()
    ...
    def readRegisterBit(self, reg_name, bit_num):
        log.debug('readRegisterBit: reg_name: ' + str(reg_name) + 'bit_num: ' + str(bit_num) )
        try:
            cmd_string = bytearray()
            cmd_string.append(READ_REGISTER_BIT+bit_num)
            cmd_string.append(reg_name)
            self.sendAPICmd(cmd_string)
            if self.sr.inWaiting()>0 :
                self.sr.flushInput()
    ...
    def readRegister16b(self, reg_name):
        log.debug('readRegister: ' + str(reg_name))
        try:
            cmd_string = bytearray()
    ...
    def writeRegisterBit(self, reg_name, bit_name, bit_value):
...
class Timer:
    ...
    def initialize(self, microseconds=1000000):
        ...
        self.parent.writeRegister(uK.TCCR1B,self.tccr1b) #set WGM13 -> set mode as phase and frequency correct pwm, stop the timer
        self.tccr1a = 0x00
        self.parent.writeRegister(uK.TCCR1A,self.tccr1a) # clear
    ...
    def setPeriod(self,microseconds):
        ...
        self.parent.writeRegister(uK.ICR1H, pwmPeriod_in_bytes[1])
        log.debug('wrifying ICR1H ... = ' + str(self.parent.readRegister(uK.ICR1H)))
        self.parent.writeRegister(uK.ICR1L, pwmPeriod_in_bytes[0])
        ...
        self.parent.writeRegister(uK.TCCR1B, self.tccr1b)
        ...
        log.debug('wrifying ICR1L ... = ' + str(self.parent.readRegister(uK.ICR1L)))
        log.debug('wrifying ICR1H ... = ' + str(self.parent.readRegister(uK.ICR1H)))
        log.debug('Check TIMER1 ... = ' + str(self.parent.readRegister16b(uK.TCNT1L)))
        log.debug('Check TIMER1 ... = ' + str(self.parent.readRegister16b(uK.TCNT1L)))
    ...
    def stop(self):
        self.parent.writeRegister(uK.TCCR1B,0x00)
